# Agent model recommendations — Verso

Модель Claude для каждого агента. Используется оркестратором (Анатолий) при делегировании. **Эта таблица — таблица-истина проекта Verso**; `.claude/agents/README.md` — тонкое зеркало. Frontmatter каждого агент-файла обязан совпадать с таблицей.
