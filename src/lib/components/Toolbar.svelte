...
<!-- The frame keeps the surface and the hairline out of the strip's edge fade. -->
<div class="toolbar-frame">
  <div class="toolbar strip-scroll" role="toolbar" aria-label={t('toolbar.label')} use:scrollFade>
    ...
    {#if markdown}
      <div
        ...
        data-mode={settings.value.editorMode}
      >
        <!-- One pill that slides, rather than two that light up in turn: the
           two halves are one setting with two positions, and a thing that
           travels between them says so. -->
        <span class="thumb" aria-hidden="true"></span>
        <button
          class="mode-btn"
          class:on={settings.value.editorMode === 'live'}
          use:tip={{ text: t('toolbar.modeLive'), shortcut: 'Ctrl+/' }}
        ...
        <button
          ...
          use:tip={{ text: t('toolbar.modeSource'), shortcut: 'Ctrl+/' }}
...
<style>
  ...
  /* Two equal columns whatever the two words translate to, so the pill can
     travel by exactly its own width. */
  .mode {
    position: relative;
    display: grid;
  ...
  .mode[data-mode='source'] .thumb {
    transform: translateX(100%);
  }

  /* The pill moving *is* the animation; with movement off it is simply where
     it belongs. */
